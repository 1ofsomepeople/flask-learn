...
# 获取历史数据，参数是数据对应时刻
@app.route('/data/od/history/', methods=["GET", "POST"]) # GET 和 POST 都可以
def getOdHistoryData():
  # 解析接口url的参数
  paramData = request_parse(request)
  dataIndex = int(paramData.get("dataIndex"))
  dataObj = {}
...
# 获取历史数据的预测值，参数是数据对应时刻与预测方法
@app.route('/data/od/pred/', methods=["GET", "POST"]) # GET 和 POST 都可以
def getOdPredData():
  # 解析接口url的参数
  paramData = request_parse(request)
  dataIndex = int(paramData.get("dataIndex"))
